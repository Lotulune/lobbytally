...
`collect-steam-catalog` 只读取服务端环境中的 `MPGS_STEAM_WEB_API_KEY`，密钥不得作为命令参数、写入 SQLite 或进入客户端包。服务端以 5 分钟的默认检查频率观察三类独立到期时间：目录同步 15 分钟、候选发现 6 小时、富化 5 分钟。候选发现同时刷新近期发售与 `comingsoon` 通道；候选池未达目标时，每个通道默认推进 10 页，可用 `MPGS_CANDIDATE_WORKER_PAGES` 在 `1..100` 内覆盖。候选池达标后，环境变量不再放大稳态扫描，每个通道刷新首页并最多推进 1 个续页。每类任务在同类 `pending` 或 `leased` 作业存在时不会再入队，因此慢目录同步不会积压并抢占候选/富化。`pending` 表示已入队但尚未领取（其中 `due_at_ms` 到期后才可领取）；`leased` 表示某个 worker 已取得限时执行权，正常情况下就是处理中，worker 中断后由租约到期恢复。使用同一数据库文件的主机必须周期执行 `run-steam-worker-once`。worker 以 SQLite 租约防止重复领取，并把当前来源任务、成功时间、下次运行、错误类别、游标和覆盖率回写到 `/admin/v1/data-status`。富化配额通常共享一次候选扫描；只有共享窗口没有某一数据维度时才执行一次定向补查，避免按维度重复扫描完整 SQLite 候选池。全量维度快照最多每 15 分钟刷新一次；当前工作和队列租约仍实时读取，避免每个分钟级任务结束后都扫描完整候选池。`enrich-steam-app` 是运营点名 AppID 的受控强制商店入库路径。不要通过网络文件系统运行该 worker；远程部署需要走受控 ingestion API。默认商店区域 `CN/schinese`。富化会优先补齐影响日历的商店详情，再同步全语言评价汇总、简体中文热门评价前 10 条与 CCU；`MPGS_ENRICH_STORE_ONLY` / `MPGS_ENRICH_SKIP_*` 在 CLI 与后台 worker 中语义一致。采集需遵守限流与 [SOURCES.md](SOURCES.md)。
...
4. `mpgs-dbtool migrate <db>`（或启动时自动 migrate）。当前最新为 `0023_search_name_trigram`：`0022_latest_review_snapshots` 回填并持续维护每个 app 的最新评论投影，`0023_search_name_trigram` 为中英文名称子串建立三元 FTS 索引并由写入触发器保持新名称可搜；Feed、详情和日历不再重复扫描评论历史。`0021_read_path_indexes` 的最新评论与 demo/playtest 反查索引仍保留。
