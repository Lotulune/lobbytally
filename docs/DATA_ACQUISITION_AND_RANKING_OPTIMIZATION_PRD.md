...
- 未发售条目优先做到“基础资料可展示且发售状态及时”；已发售游戏优先做到“推荐证据
  完整且持续刷新”。
- 推荐排序只对全局前 5 名显示推荐名次；任何标量排序都不显示推荐名次。
- 适配指数表示相对适配程度，推荐顺序表示经过多样性等约束后的最终编排，两者不再用
  相似文案暗示它们等价。
...
适配指数回答“在本次请求返回窗口中，这个游戏的相关性得分相对处于什么位置”。当前实现
使用 `relevance_score` 的上下文百分位，并且只有数据置信度和有效特征数量达标时才显示。
它不是概率，也不适合跨分区、跨用户、跨日期或跨分页直接比较。

推荐顺序回答“综合适配后，产品最终把哪些游戏放在前面”。推荐排序还会考虑 MMR 多样性、
探索位、分区规则、个性化、想玩信号和稳定 tie-break。因此高适配指数不保证一定排在更低
指数之前，这是设计行为，不是算法矛盾。

CCU、评价数、发售日期和适配指数排序都是单字段标量排序。此时 API 的 `rank` 只是列表
位置，不能称为推荐名次。

### 5.2 展示规则

- UI 文案将“推荐指数”改为“适配指数”，API 字段 `recommendation_index` 暂时保留以避免
  破坏兼容性。
- `sort=recommended` 且全局 `rank <= 5`：显示“推荐第 N”及可用的适配档位/指数。
- `sort=recommended` 且 `rank > 5`：不显示序数，只显示可用的适配档位/指数；资料不足时
  显示“资料较少，待观察”。
- `sort=ccu/reviews/release_date/fit_index`：一律不显示“推荐第 N”。可显示当前排序值；
  `fit_index` 可继续显示适配指数。
