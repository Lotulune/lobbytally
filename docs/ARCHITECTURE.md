...
推荐器的 canonical ordering 为：

1. 候选召回与分区生成：最近发售、未来 30 天/Demo、人气老游、经典老游。
2. 客观硬过滤：类型、明确平台/人数条件、服务状态等；长期合作/竞技口味留给软排序。
3. 确定性分区、个性化、反馈和保守社区信号；按连续相关性稳定排序。
4. 自然语言场景可在合法候选内执行最多 15% 的 AI 数值融合；模型数组顺序无排序权限。
5. 结构化 MMR/探索重排并输出最终 `rank` 和 `slot_reason`。

普通 Feed 不调用 AI。`rules-0.3` 的通用自然语言请求先从已同步的 FTS/向量索引取得最多 300 个 RRF AppID，再从四分区完整排名集合中准入仍通过资格、请求硬条件和负反馈过滤的命中；严格相关性补足去重联合池到最多 300。显式分区请求保持单区范围。可选 AI 只做最多 15% 的数值融合，随后统一执行一次全局 MMR；内部 Feed 不落归因 run，最终响应只保存一个自然语言 run。
