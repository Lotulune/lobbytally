...
- `algorithm_version`：正在执行的代码/公式版本，当前为 `rules-0.3.1`。
...
**原则（治本）**：新游不可能一上线就都有高评价。发现通道若以评价量/好评排序当主入口，等于系统性漏掉刚发售的联机作；这是 **发现策略错误**，不是「Feed 排序藏起来」或「前端展示延迟」。

**问题（已线上踩坑）**：联机候选来自 Steam 商店搜索 `category2=1`。旧版 `Reviews_DESC` 下，「刚发售、评价很少/为零」的典型派对/联机新作（例如 2026-07-30《机械狂欢》app 4108000）几乎不会出现在前几页 → **发现阶段漏检**。用户侧表现为「近期发售停在旧日期、点名新游搜不到」。

**已落地基线（`store-search-0.2` / 生产补丁，先止血）**：

- 默认排序改为 `Released_DESC`（按发售日新→旧），让新游先进池。
- 游标键改为 `steam_store_search:multiplayer:released_desc`，避免沿用旧 Reviews 偏移。
- 调度在候选数已达目标时仍刷新首页，以捕获最新发售。
- Feed/日历侧「近期」以当前商店 `release_date` 为准（勿用首次出现的 `first_release` MIN 埋掉改档日期）。

**后续算法优化（未做，记入迭代；勿再把「口碑排序」当发现唯一通道）**：

1. **双通道合并**：`Released_DESC`（新游）+ `Reviews_DESC`（口碑）分页并集，按 app_id 去重——新游靠发售日进池，老盘靠口碑补全，防止只追新或只追热。
2. **地区/语言**：`cc`/`l` 与运营区一致（如 `cn`/`schinese`），减少英文区列表偏差。
3. **新游保送配额**：`recent_release` / 日历窗口内，对「发售 ≤14 天且联机证据成立」保留最低曝光比例，避免推荐分/评价代理再次把新游压没。
4. **日历与发现对齐**：即将发售有日期候选过少时，监测页应提示「发现偏旧」而非仅显示红条。
5. **强制入库运维路径**：对运营点名的 app_id（如 4108000）提供一键 store enrich，不依赖搜索页运气。
6. **评价信号分层**：评价量/好评只作排序与解释特征，不作为「能否进入候选池」的硬门槛；零评新游只要商店联机分类/证据成立即可进池。

改动发现排序或配额时必须更新 `adapter_version` / `config_version`，并在黄金集或抽检集中验证「新发售联机作可被发现」（至少含发售后 ≤7 天、评价 <50 的样本）。
